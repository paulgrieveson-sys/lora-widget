...
- **FIX (2026-09-19) — sanitisePhone was destroying valid numbers.** It stripped every non-digit from the WHOLE string, so a trailing word containing a digit joined the number: `"01614428740 option 1"` became the 12-digit `016144287401`, failed the 11-digit landline test, and was dropped. **Cost a real lead:** Alison Dermott (Commercial unit, Manchester, guide £1,035-£2,590, timeline within 30 days) gave her landline on 11 Sep, Lora read it back correctly in chat and confirmed "Tuesday 10am", but the contact saved with NO phone and the call was never made. Hers was the only lead with a rejected number, and the only one who had booked. Fix: `normaliseUKPhone(digits)` split out (the original four branches, unchanged); `sanitisePhone` now tries the whole string first for back-compat, then falls back to matching phone-shaped digit runs (allowing spaces, dashes and brackets, with an optional leading +) and returns the first that validates. Covers "option 1", "ext 2", "press 3" and a number stated mid-sentence. Garbage still rejected. **Note the booking flow itself is still the real problem — see below.**
- **⚠ LORA DOES NOT ACTUALLY BOOK ANYTHING (confirmed 2026-09-19).** There is no calendar and no appointment API call in the widget. On "yes" she takes a preferred day/time as FREE TEXT, sets `callBooked:true`, tags the contact `call booked` and moves the opportunity to "Appointment booked". **JDCM GHL has 0 appointments on the Intro Call calendar (`vJHVbYMmp7Qy7srpP37J`) for all of 2026**, against 2 opportunities sitting in Appointment booked. Consequences seen live: (a) Zain, 10 Aug, was marked booked with `call_date_time` = "TBD" and then SENT a reminder reading *"your consultation call is coming up, TBD"*; (b) Alison was promised a call with no phone number on file. Nothing checks that a time was agreed or a number exists before promising. **Fix direction: hand over the real Intro Call calendar link instead of promising a call** — prompt-only change, gives a real slot, confirmation, reminder, and collects the phone itself.
